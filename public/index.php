...
$router->group('/add', function (Router $router): void {
  $router->post('/users/[name:%t]', TestController::class . ':store')->addMiddleware(TestMiddleware::class)->alias('user_by_name', ['name' => 'Adriano']);
...
$router->group('/add', function (Router $router): void {
  $router->post('/posts/[name:%t]/text/[id:%d]', TestController::class . ':store')->addMiddleware(TestMiddleware::class);
